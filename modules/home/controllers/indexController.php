...
function orderAction(){
    ...
    if (isset($_POST['btn-order'])) {
        ...
        if($payment=='payment-momo'&&empty($error)){
            load_view('paymomo');
        }
        else if($payment=='payment-momoatm'&&empty($error)){
            $fullname = $_POST['fullname'];
            $email = $_POST['email'];
            $phone = $_POST['phone'];
            $province = $_POST['province'];
            ...
            $_SESSION['order'] = compact('fullname', 'email', 'phone', 'address', 'note', 'payment');
            $error['success'] = "Đặt hàng thành công!";
            unset($_SESSION['cart']);
            header("Location: " . base_url("modules/cart/views/paymomoatm.php"));
            exit();
        }
        // check not error
        if (empty($error)&&$payment!='payment-momoatm'&&$payment!='payment-momo') {
            $address = $commune.', '.$district.', '.$province;
            $data_order = array(
            'order_code' => 'DT'.time(),
